document.addEventListener('DOMContentLoaded', () => {
            ...
            if (item.type === 'video') {
                ...
                card.innerHTML = `
                    <div class="thumbnail-container">
                        ...
                        <button class="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/60 text-white opacity-0 group-hover:opacity-100 transition-all hover:bg-accent-red z-10 add-pl-btn" onclick="event.stopPropagation(); window.addToPlaylist(${JSON.stringify(item).replace(/'/g, "&apos;")})">
    ...
    async function loadVideoFromHash() {
        ...
        try {
            ...
            getEl('player-description').innerHTML = `<div class="whitespace-pre-wrap text-sm leading-relaxed">${data.description}</div><button id="player-add-btn" class="mt-6 bg-brand-border px-4 py-2 rounded-full text-xs hover:bg-accent-red transition-all">Add to Playlist</button>`;
